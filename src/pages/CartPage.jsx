...
import ChooseDishButton from "../components/Buttons/ChooseDishButton.jsx";
...
function CartPage() {
    const {cartItems, addToCart, decreaseQuantity, clearCart, getCartTotal, removeFromCart} = useContext(CartContext);
                ...
                cartItems.length > 0 ? (
                    <>
                        <div className={"flex justify-between"}>
                            ...
                            <button
                                className={"py-2 px-6 bg-F06D00 border-2 border-F06D00 rounded-2xl text-xl font-bold hover:bg-white hover:text-F06D00"}
                                onClick={() =>
                                    clearCart()
                                }>
                        ...
                        <div className={"space-y-5"}>
                            {
                                cartItems.map((cartItem) => {
                                    return (
                                        <div
                                            className={"relative flex justify-between border-b-2 py-10 border-b-black"}>
                                            <div className={"flex gap-x-10"}>
                                                <ThreeSideRoundedFrame bgColor={"BCD1BD"} className={"w-44 h-44"}>
                                                    <img src={`/images/dishes/${cartItem.img_name}`} alt=""/>
                                                </ThreeSideRoundedFrame>
                                                <div>
                                                    <h4>{cartItem.name}</h4>
                                                    <div className={"flex flex-col gap-y-4"}>
                                                        <span>Вес блюда: {cartItem.weight * cartItem.quantity} - {cartItem.kilocalories * cartItem.quantity} Ккал</span>
                                                        <span>Белки{cartItem.proteins * cartItem.quantity} гр.</span>
                                                        <span>Жиры {cartItem.fats * cartItem.quantity} гр.</span>
                                                        <span>Углеводы {cartItem.carbons * cartItem.quantity} гр.</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <PriceWithRubleSymbol priceSell={cartItem.price_sell * cartItem.quantity}/>
